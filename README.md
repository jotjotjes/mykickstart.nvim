...
that moves from a single file to a multi file configuration.*
...
- Why is the kickstart `init.lua` a single file?
  ...
  - The main purpose of kickstart is to serve as a teaching tool and
    a reference configuration that someone can easily use to `git clone`
    ...
    A fork of kickstart that does this while maintaining the
...
Installation may require installing build tools and updating
the run command for `telescope-fzf-native`
